 ...
 * allowing for annotation of one peak with multiple genes using toolset called **GREAT**. This is usually done to annotate enhancer peaks.
...
The specific set of peaks we choose will depend on our question. In our case, we just performed differential analysis on our consensus peak set, so we can use the genes that go up in W6 vs W0.
...
```{r,eval=T,echo=T, eval=T, echo=T, warning=FALSE}
W6MinusD0_gr_main <- W6MinusD0_gr[seqnames(W6MinusD0_gr) %in% paste0("chr", c(1:19, "X", "Y", "M"))]
...
This will give us our gene list to test for functional enrichment.
...
For this Sox9 CUT&RUN dataset this is is not many peaks, resulting in a small gene list. 
```{r}
